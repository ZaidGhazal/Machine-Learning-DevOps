...
def go(args):
    ...
    wandb.init(project="exercise_1", job_type="upload_file")
    ...
    # HINT: you can use args.artifact_name to reference the parameter artifact_name
        artifacte = wandb.Artifact(name=args.artifact_name, type=args.artifact_type, description=args.artifact_description)
    ...
    # ``artifact.add_file``, and log the artifact to the run using ``run.log_artifact``.
        artifacte.add_file(args.input_file)
        run.log_artifact(artifacte)
